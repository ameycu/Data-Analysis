...
def analysis_five():
	...
	for m in d['items']:
		if m['actor_1_name'] in actor.keys():
			actor[m['actor_1_name']][0] += float(m['movie_facebook_likes'])
			actor[m['actor_1_name']][1] += 1
		if m['actor_2_name'] in actor.keys():
			actor[m['actor_2_name']][0] += float(m['movie_facebook_likes'])
			actor[m['actor_2_name']][1] += 1
		if m['actor_3_name'] in actor.keys():
			actor[m['actor_3_name']][0] += float(m['movie_facebook_likes'])
	...
	ax.set_title('Average Facebook likes of Actors')
	ax.set_xlabel('Actors')
	ax.set_ylabel('Facebook Likes')
	count = 0
	for actor, value in actor_average.items():
		ax.annotate(actor, xy=(count, value), xytext=(count, value+4000),
